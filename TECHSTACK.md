...
│   │   │       ├── Pagination.jsx         ← default export Pagination (windowed page-number bar ‹ Prev / … / Next ›; renders null when totalPages <= 1) + named export FillerRows (invisible &nbsp; spacer rows padding a short/empty page to constant height; lines={1|2} matches row height); used by all list, dashboard, and reports tables
│   │   │       └── EmptyState.jsx         ← empty-state <tr> for list tables: centered gray message + outline "empty tray" icon below; props colSpan + message; used by Dashboard (3 branches) + Receive Order / Transfer Request / Reconciliation list pages
│   │   ├── context/
...
│   │   │   ├── usePagination.js           ← client-side pagination hook usePagination(items, { pageSize, resetKey }) → { page, setPage, totalPages, pageItems, fillerCount }; resets to page 1 during render when resetKey changes; fillerCount pads a short page to pageSize (or EMPTY_STATE_FILLER_ROWS when the list is empty). Page-size constants: DEFAULT_PAGE_SIZE=9 (dashboard/inventory, taller two-line rows), LIST_PAGE_SIZE=15 (RO/TRF/RC list pages, single-line rows), REPORTS_PAGE_SIZE=14 (reports, no footer/search row), EMPTY_STATE_FILLER_ROWS=11 (blank rows under a "No X yet" message). 12 consumers.
