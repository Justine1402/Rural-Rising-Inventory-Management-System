...
transfer-destination batches. At TRF accomplish, one new `SKU-[DEST_WH]-[SKU_SEQ]-[BATCH_SEQ]`
batch is created in the destination warehouse **per source batch deducted** by the cascade
plan — not one per item. The batch sequence is scoped per product+warehouse combination.
