...
- Removed stale "Step 14 reports will reuse this" forward-looking comment from `src/utils/reconciliationFormat.js`. The reports render the pre-formatted `products_with_discrepancy` integer from `ReportController` directly, so the utility was never reused by Step 14.
- Annotated four currently-unused inverse `belongsTo` relations on `IssueProductBatchDeduction` (`issueProductItem`, `stockInUse`) and `TransferRequestBatchDeduction` (`transferRequestItem`, `stockInUse`) with intentional-retention comments. These are kept for a planned backward-traversal feature (batch deduction log entry → parent transfer/issue item, → source batch). Feature integration UX is being worked out; relations are wired now to avoid re-adding them later. Not dead code — do not remove in future audits.
- The 2026-05-16 entry grouped `StockInUse` with the two `BatchDeduction` models as having unused inverse relations. This audit verified that's incorrect for `StockInUse`: its `product()` and `warehouse()` relations are actively used by `ReportController` (eager-loaded via `StockInUse::with(['product', 'warehouse'])`). No action needed on that model; the deferred-cleanup list now applies only to the two `BatchDeduction` models.
...
### Production Decommissioning — 2026-08-01

- The client (Rural Rising Philippines) will no longer be using the deployed system;
  the project is being decommissioned from production. The codebase itself is
  unaffected — only deployment status has changed.
- Hetzner server `ruriims-prod` (CPX22, Singapore, `5.223.86.207`) was permanently
  deleted. No other billable Hetzner resources (volumes, floating IPs, snapshots,
  backups) remained on the account.
- The Cloudflare DNS zone for `ruriims.com` had its A record removed; the domain no
  longer resolves to any server. Zone-level Cloudflare account settings were left
  untouched.
- The Namecheap domain registration for `ruriims.com` remains active through
  **2027-06-07**, with auto-renew disabled for both the domain and Domain Privacy —
  it will lapse naturally at expiry rather than being charged again.
- The codebase remains preserved in the GitHub repository
  (`Justine1402/Rural-Rising-Inventory-Management-System`) as the permanent record.
